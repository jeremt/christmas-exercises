/**
 * @typedef {import('./runTests').TestResult} TestResult
 */
...
this.__codePassport = {
    ...
    /**
     * Call the given function asynchron without tests.
     * @param {Function} func The function to call.
     * @param {unknown[]} params The parameters to pass to the function.
     * @example await __codePassport.callAsyncFunc(sendPrompt, []);
     */
    async callAsyncFunc(func, params) {
        await func?.(...params);
        this.tests.push({
            type: 'callFunc',
            success: true,
    ...
    testFunc(func, params, expected) {
        ...
        this.tests.push({
            ...
            exists: func !== undefined,
            funcName: func.name,
            params: params.map((p) => (typeof p === 'function' ? p.toString() : p)), // 💩
            result,
            expected,
        });
    },
    ...
    async testAsyncFunc(func, params, expected) {
        // 💩 maybe simply using await in testFunc is enough
     ...
     * @example __codePassport.testThrow(recycle, ['🥝'], `🥝 n'est pas recyclable`);
     */
     ...
     * @example __codePassport.testHTML(__codePassport.ingredients, `<li>blanc d'œuf (36g)</li><li>sucre en poudre (27g)</li><li>poudre d'amande (45g)</li><li>sucre glace (47g)</li><li>cacao en poudre (5g)</li>`);
     */
    testHTML(el, expected = '') {
        if (!el) {
            this.tests.push({
                type: 'assert',
                success: false,
                failedMessage: 'Le contenu du HTML ne correspond pas à ce qui est attendu.',
                successMessage: 'Le HTML correspond au résultat attendu.',
            });
            return;
        }
    ...
    cssPxToZero: (value) => (value === '0px' ? '0' : value),
     ...
     */
    cssPxToRem: (value) => (!/[0-9]+/.test(value) ? value : `${parseInt(value) / parseFloat(getComputedStyle(document.documentElement).fontSize)}rem`),
     ...
     *
     * ⚠️ This function only handles a small subset of css colors, it returns the
     * rgb color otherwise.
     *
    ...
    async triggerEvent(el, event, newValue = undefined) {
        el.value = newValue;
        // target must be set like so because we manually dispatch the event 😭
